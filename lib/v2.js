...
const { OverflowingBuffer } = require('./internal/buffer')
const {
  ERR_HMAC_MISMATCH,
...
class Encryptor extends Transform {
  ...
  constructor (credential, options) {
    ...
    if ('password' in credential) {
      ...
      credential.password = Buffer.alloc(
        credential.password.length,
        credential.password
      )
      credential.encryptionSalt ??= randomBytes(SALT_SIZE)
      credential.hmacSalt ??= randomBytes(SALT_SIZE)
      credential.encryptionKey = pbkdf2Sync(
        credential.password,
    ...
    assert('encryptionKey' in credential, new ERR_INVALID_CREDETIAL_TYPE())
    credential.iv ??= randomBytes(IV_SIZE)
    this.#pendingHeader = Buffer.concat([this.#pendingHeader, credential.iv])
    this.#engine = createCipheriv(
      CIPHER_ALGORITHM,
      credential.encryptionKey,
      credential.iv
    )
    this.#hmac = createHmac(HMAC_ALGORITHM, credential.hmacKey).update(
      this.#pendingHeader
    )
  }
  ...
  update (data, inputEncoding, outputEncoding) {
    const result = this.#handle(this.#engine.update(data, inputEncoding))
...
class Decryptor extends Transform {
  ...
  #hmac
  static canDecrypt (preamble, inputEncoding) {
    if (typeof preamble === 'string') { preamble = Buffer.from(preamble, inputEncoding) }
    assert(preamble.length >= MAGIC.length, new ERR_MESSAGE_TOO_SHORT())
  ...
  final (outputEncoding) {
    ...
    assert(originalHmac.length === HMAC_SIZE, new ERR_MESSAGE_TOO_SHORT())
    const computedHmac = this.#hmac.digest()
    assert(computedHmac.equals(originalHmac), new ERR_HMAC_MISMATCH())
  ...
  #parseHeader (data) {
    ...
    const shouldCheck = (length) =>
      previousLength < (offset += length) && this.#header.length >= offset
    if (shouldCheck(MAGIC.length)) {
      assert(
        this.#header.subarray(0, MAGIC.length).equals(MAGIC),
        new ERR_UNKNOWN_HEADER()
      )
    }
    if (shouldCheck(1)) {
      assert(
        'password' in this.#credential ? 1 : 0,
        new ERR_INVALID_CREDETIAL_TYPE()
      )
    }
    if ('password' in this.#credential && shouldCheck(2 * SALT_SIZE)) {
      const encryptionSalt = this.#header.subarray(
        offset - 2 * SALT_SIZE,
        offset - SALT_SIZE
      )
      const hmacSalt = this.#header.subarray(offset - SALT_SIZE, offset)
      this.#credential.encryptionKey = pbkdf2Sync(
      ...
      this.#credential.hmacKey = pbkdf2Sync(
        this.#credential.password,
        hmacSalt,
        PBKDF2_ITERATIONS,
        KEY_SIZE,
    ...
    if (shouldCheck(IV_SIZE)) {
      ...
      this.#engine = createDecipheriv(
        CIPHER_ALGORITHM,
        this.#credential.encryptionKey,
        iv
      )
