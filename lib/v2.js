  ...
  constructor(credential, options) {
    ...
    if ("password" in credential) {
      if (typeof credential.password !== "string")
        credential.password = credential.password.toString();
...
class Decryptor extends Transform {
  ...
  static canDecrypt(preamble, inputEncoding) {
    if (typeof preamble === "string")
      preamble = Buffer.from(preamble, inputEncoding);
  ...
  #parseHeader(data) {
    ...
    if (!this.#hmac && "hmacKey" in this.#credential)
      this.#hmac = createHmac(HMAC_ALGORITHM, this.#credential.hmacKey);
