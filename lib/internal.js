...
const errors = {
  ERR_HMAC_MISMATCH: 'HMAC mismatch',
  ERR_UNKNOWN_HEADER: 'Unknown header',
  ERR_MESSAGE_TOO_SHORT: 'Message too short',
  ERR_INVALID_CREDETIAL_TYPE: 'Invalid credential type'
}
class OverflowingBuffer extends Transform {
  ...
  update(data, inputEncoding, outputEncoding) {
    ...
    if (data.length >= this.capacity) {
      ...
      this.#buffer = Buffer.concat([this.#buffer.subarray(data.length - this.capacity), data])
    } else {
